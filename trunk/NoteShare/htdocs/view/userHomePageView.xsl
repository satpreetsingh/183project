...
<xsl:key name="session-by-uni" match="SessionUserItem" use="University" />
...
<xsl:template match="UserSessionList">
	<xsl:for-each select="UserSessionItem[count(. | key('session-by-uni', University)[1]) = 1]">
		<div>
			<xsl:sort select="University" />
	   		<h3><xsl:value-of select="University" /></h3>
	   		<ul>
		   		<xsl:for-each select="key('session-by-uni', University)">
		   			<li>
			   			<a target="_top">
